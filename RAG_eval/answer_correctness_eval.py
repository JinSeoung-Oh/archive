...
class AnswerCorrectness(MetricWithLLM, MetricWithEmbeddings):
    ...
    def _compute_statement_presence(
        ...
        return score, prediction.TP, prediction.FP, prediction.FN
    ...
    async def _ascore(self, row: t.Dict, callbacks: Callbacks, is_async: bool) -> float:
        ...
        if not all([val == [] for val in statements.values()]):
            ...
            f1_score, reason_TP, reason_FP, reason_FN = self._compute_statement_presence(answers)
        else:
            f1_score = 1.0
            reason_TP = 'Cannot evaluate, becuase LLM return empty response'
            reason_FP = 'Cannot evaluate, becuase LLM return empty response'
            reason_FN = 'Cannot evaluate, becuase LLM return empty response'
        ...
        result_dict['TP_reason'] = reason_TP
        result_dict['FP_reason'] = reason_FP
        result_dict['FN_reason'] = reason_FN
